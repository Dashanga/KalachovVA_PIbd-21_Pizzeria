...
namespace ForgeServiceImplementList.Implementations
{
    public class MainServiceList : IMainService
    {
        ...
        public void TakeOrderInWork(OrderBindingModel model)
        {
            ...
            if (source.Orders[index].Status != OrderStatus.Received)
            {
                throw new Exception("Заказ не в статусе \"Принят\"");
            }
            source.Orders[index].DateImplement = DateTime.Now;
            source.Orders[index].Status = OrderStatus.Processing;
        }
        public void FinishOrder(OrderBindingModel model)
        {
            ...
            if (source.Orders[index].Status != OrderStatus.Processing)
            {
