...
namespace PizzeriaServiceImplementDB.Implementations
{
    public class ReportServiceDB : IReportService
    {
        ...
        public void SaveProductPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Pizzas.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].PizzaName;
        ...
        public List<StorageLoadViewModel> GetStocksLoad()
        {
            ...
            (stock, stockCompList) =>
                new StorageLoadViewModel
                {
                    ...
                    TotalCount = stockCompList.Sum(r => r.StorageIngredientCount),
                    Ingredients = stockCompList.Select(r => new Tuple<string,
                        int>(r.Ingredient.IngredientName, r.StorageIngredientCount))
                })
                .ToList();
        }
 ...
 excel.Workbooks[1].Save();
            ...
            public List<CustomerOrdersModel> GetClientOrders(ReportBindingModel model)
            {
                return context.PizzaOrders
                .Include(rec => rec.Customer)
                .Include(rec => rec.Pizza)
                .Where(rec => rec.CreationDate >= model.DateFrom &&
               rec.CreationDate <= model.DateTo)
            ...
            public void SaveClientOrders(ReportBindingModel model)
            {
                ...
                {
                    File.WriteAllBytes("TIMCYR.TTF", Properties.Resources.TIMCYR1);
                }
               ...
               iTextSharp.text.Font.BOLD);
                ...
                {
                    cell = new PdfPCell(new Phrase(list[i].FullName, fontForCells));
                    table.AddCell(cell);
                    cell = new PdfPCell(new Phrase(list[i].CreationDate, fontForCells));
                    table.AddCell(cell);
                    cell = new PdfPCell(new Phrase(list[i].PizzaName, fontForCells));
                    table.AddCell(cell);
                    cell = new PdfPCell(new Phrase(list[i].PizzaCount.ToString(), fontForCells));
                    cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                    table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].TotalCost.ToString(), fontForCells));
                    cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                    table.AddCell(cell);
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list.Sum(rec => rec.TotalCost).ToString(),
               fontForCellBold))
                {
                    HorizontalAlignment = Element.ALIGN_RIGHT,
                    Border = 0
